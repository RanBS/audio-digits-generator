...
dataset = DigitsAudioDataset('../../dataset/train_spectograms')
...
def sample_image(n_row, batches_done, dataset):
    """Saves a grid of generated digits ranging from 0 to n_classes"""
    # Sample noise
    z = Tensor(np.random.normal(0, 1, (n_row ** 2, opt.latent_dim)))
    # Get labels ranging from 0 to n_classes for n rows
    labels = Tensor(np.array([num for _ in range(n_row) for num in range(n_row)]))
    labels = labels_to_one_hots(labels, 10)

    phases_idxs_to_sample = [1350 +i*2700 for i in range(10)]
    phases = torch.cat([dataset[i]['phase'] for i in phases_idxs_to_sample]*n_row).view(n_row**2, -1).type(Tensor)

    conds = torch.cat((phases, labels), -1)
...
for epoch in range(opt.n_epochs):
    ...
    for i, sampled_batch in enumerate(dataloader):
        ...
        phases = sampled_batch['phase']
        phases = phases.view(phases.shape[0], -1)
        amps = sampled_batch['amp']
        labels = sampled_batch['label']

        # Move to GPU if necessary
        real_imgs = amps.type(Tensor)
        phases = phases.type(Tensor)
        ...
        z = Tensor(np.random.normal(0, 1, (amps.shape[0], opt.latent_dim)))
        ...
        if i % opt.n_critic == 0:
            ...
            fake_validity = discriminator(fake_imgs, conds)
            g_loss = -torch.mean(fake_validity)# + alpha * mse_loss(fake_imgs, real_imgs)
            ...
            if batches_done % opt.sample_interval == 0:
                 sample_image(opt.n_classes, batches_done, dataset)
