...
def sample(label, generator_model, opt, path_to_save):
    ...
    d[0, :, :] = librosa.db_to_amplitude((d[0, :, :] * 0.5 + 0.5) * (22.376 + 97.002) - 97.002)
    d[1, :, :] = (d[1, :, :] * 0.5 + 0.5) * (2 * np.pi) - np.pi

    rec_spec = np.zeros((256, 128))*1j
    rec_spec[0:128] = d[0, :, :] * np.exp(1j*d[1, :, :])
...
if __name__ == "__main__":
    ...
    for i, label in enumerate(labels_to_sample):
        ...
        #sample_amplitude(label, generator_model, opt, '../../dataset/data_for_metrics/exp 2/' + str(label) + '/' + str(i))
